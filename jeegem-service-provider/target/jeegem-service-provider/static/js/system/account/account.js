	...
	$('#addBtn').on('click', function(e) {
		...
		JEEGEM.Model.edit("auDiv","新增",function(){
			 if(JEEGEM.Validate.form("auForm")){
				 var that =$(this);
				 JEEGEM.Ajax.doRequest("auForm",jeegempath +'/backstage/account/add',null,function(data){
				     that.dialog("close");      
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JEEGEM.Model.confirm("确认要删除选中的数据吗?",function(){	
				JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/delBatch',{chks:chks.toString()},function(data){
					JEEGEM.Model.info(data.resMsg,function(){search();});
				});
			});		
		}		
...
function loadRoleTree(){
	JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/roleTree',null,function(data){
...
function getbaseList(init){
	...
	JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/account/findByPage',null,function(data){
		 $("#baseTable tbody").empty();
    		 ...
    		 if(results!=null&&results.length>0){
        		 ...
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 html+="<td class='center hidden-480'>"+JEEGEM.Object.notEmpty(l.email)+"</td>";
            		 ...
            		 html+="<td class='center hidden-480'>"+JEEGEM.Date.Default(l.loginLog.loginTime)+"</td>";
            		 html+="<td class='center hidden-480'>"+JEEGEM.Object.notEmpty(l.loginLog.loginIP)+"</td>";
...
function check(accountId){
	...
	JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/roleTree',null,function(data){
		$.fn.zTree.init($("#roleTree"),{view:{dblClickExpand:false,selectedMulti:false,nameIsHTML:true},data:{simpleData:{enable: true}},callback:{onClick:clickRole}},data.obj);
		JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/find',{accountId:accountId},function(data){
		    setForm(data);
...
function del(accountId){
	JEEGEM.Model.confirm("确认删除吗？",function(){	
		JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/del',{accountId:accountId},function(data){
			JEEGEM.Model.info(data.resMsg,function(){search();});
		});
	});
}
	...
	JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/roleTree',null,function(data){
		...
		JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/account/find',{accountId:accountId},function(data){
		    ...
		    JEEGEM.Model.edit("auDiv","修改",function(){
		    	if(JEEGEM.Validate.form("auForm")){
					var that =$(this);
					JEEGEM.Ajax.doRequest("auForm",jeegempath +'/backstage/account/update',null,function(data){
					    that.dialog("close");
					    JEEGEM.Model.info(data.resMsg,function(){search();});	
					});
				}	
	...
	$("#resetPwdDiv").removeClass('hide').dialog({resizable: false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>密码重置</h4></div>",title_html: true,
		buttons: [
		  {  
			 ...
			 click: function() {
				 if(JEEGEM.Validate.form("resetPwdFrom")){
					 var that =$(this);
					 JEEGEM.Ajax.doRequest("resetPwdFrom",jeegempath +'/backstage/account/resetPwd',null,function(data){
						 that.dialog("close");
						 JEEGEM.Model.info(data.resMsg,function(){search();});		
					 });
				 }		
...
function setForm(data){
	...
	$("#auForm input[name$='loginName']").val(JEEGEM.Object.notEmpty(l.loginName));
	$("#auForm input[name$='loginName']").prop("disabled",true); 
	$("#auForm input[name$='name']").val(JEEGEM.Object.notEmpty(l.name));
	$("#auForm input[name$='email']").val(JEEGEM.Object.notEmpty(l.email));
