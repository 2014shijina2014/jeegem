$(function () {
    JEEGEM.Dict.setSelect("selectPosType","positionType",'1');
	loadOrgTree();
	loadPreOrgTree();
	//新增职务
	$('#addPosBtn').on('click', function(e) {
		...
		e.preventDefault();
		cleanPosForm();
	              JEEGEM.Model.edit("auPosDiv","新增职务",function(){
			 ...
			 if(JEEGEM.Validate.form("auPosForm")){
				 var that =$(this);
				 JEEGEM.Ajax.doRequest("auPosForm",jeegempath +'/backstage/org/position/addPos',null,function(data){	
					that.dialog("close"); 
		                      JEEGEM.Model.info(data.resMsg,function(){refreshOrgTree();getOrgList(1);});    
				 }); 
			 }	
		});
	});
	...
	$('#arrangeAccBtn').on('click', function(e) {
		...
		var posId=$("#basePosForm input[name$='posId']").val();
	              JEEGEM.Model.edit("arrangeAccDiv","安排人员",function(){
			var chks =[];    
			$('#arrangeAccTable input[name="ids"]:checked').each(function(){    
				chks.push($(this).val());    
			});
			if(chks.length==0) {
				 JEEGEM.Model.info("您没有选择任何人员!"); 
			}else{
				var that =$(this);
			              JEEGEM.Model.confirm("确认选中的人员吗?",function(){	
				              JEEGEM.Ajax.doRequest("",jeegempath +'/backstage/org/position/arrangeAcc',{posId:posId,chks:chks.toString()},function(data){
						that.dialog("close"); 
					              JEEGEM.Model.info(data.resMsg,function(){getPosList(1);});
					});
				});		
			}		
		});
...
function refreshOrgTree(){
	 ...
	 loadPreOrgTree();
	 JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/position/getOrgAndPositionTree',null,function(data){
			//设置
...
function loadPreOrgTree(){
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/position/getPreOrgTree',null,function(data){
		//设置
...
function getOrgList(init){
	if(init==1)$("#baseOrgForm .pageNum").val(1);	
              JEEGEM.Model.loading();
              JEEGEM.Ajax.doRequest("baseOrgForm",jeegempath +'/backstage/org/position/findOrgByPage',null,function(data){
		 $("#baseOrgTable tbody").empty();
    		 var obj=data.obj;
        	 var list=obj.list;
    		 ...
    		 if(results!=null&&results.length>0){			
        		 ...
        		 JEEGEM.Page.setPage("baseOrgForm","orgpageing",pageSize,pageNum,totalRecord,"getOrgList");
        	 }else{
...
function cleanPosForm(){
              JEEGEM.Tags.cleanForm("auPosForm");
...
function setPosForm(data){
	...
	if(JEEGEM.Object.notNull(nodes)&&nodes.length>0){
		treeObj.selectNode(nodes[0]);
...
function checkPos(id){
	cleanPosForm();
              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/position/findPos',{id:id},function(data){
    	setPosForm(data);    
                  JEEGEM.Model.check("auPosDiv","查看职务");   
	});
}
function editPos(id){
	//清空表单
	cleanPosForm();	
              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/position/findPos',{id:id},function(data){
			setPosForm(data);    
    	              JEEGEM.Model.edit("auPosDiv","修改职务",function(){
    			 var selectType=$("#auPosForm select[name$='type']").val();
    			 if(!GEM.Object.notNull(selectType)){
    				$("#auPosForm select[name$='type']").tips({side:1,msg : "请选择！",bg:'#FF2D2D',time:1});
    				return;
    			 ...
    			 if(JEEGEM.Validate.form("auForm")){
					 var that =$(this);
					 JEEGEM.Ajax.doRequest("auPosForm",jeegempath+'/backstage/org/position/updatePos',null,function(data){
			        	that.dialog("close");
			                      JEEGEM.Model.info(data.resMsg,function(){refreshOrgTree();getOrgList();});      				        		 
					 });
				}
    		});  
	});
...
function delPos(id){
              JEEGEM.Model.confirm("确认删除职务吗？",function(){	
	              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/position/delPos',{id:id},function(data){
		              JEEGEM.Model.info(data.resMsg,function(){refreshOrgTree();getOrgList();});
		});
	});	
}
function cleanArrangeAccForm(){
              JEEGEM.Tags.cleanForm("arrangeAccForm");
}
function loadArrangeAcc(init){
	if(init==1)$("#arrangeAccForm .pageNum").val(1);	
              JEEGEM.Model.loading();
	var posId=$("#basePosForm input[name$='posId']").val();
              JEEGEM.Ajax.doRequest("arrangeAccForm",jeegempath +'/backstage/org/position/findArrangeAccByPage',{id:posId},function(data){
		 $("#arrangeAccTable tbody").empty();
        	 ...
        	 var html="";
	   		 if(results!=null&&results.length>0){			
	       		 ...
	       		 JEEGEM.Page.setSimPage("arrangeAccForm","arrangeAccpageing",pageSize,pageNum,totalRecord,"loadArrangeAcc");
	       	 }else{
...
function getPosList(init){
	if(init==1)$("#basePosForm .pageNum").val(1);	
              JEEGEM.Model.loading();
              JEEGEM.Ajax.doRequest("basePosForm",jeegempath +'/backstage/org/position/findPosByPage',null,function(data){
		 $("#basePosTable tbody").empty();
    		 var obj=data.obj;
        	 var list=obj.list;
        	 var results=list.results;
    		 ...
    		 if(results!=null&&results.length>0){			
        		 ...
        		 JEEGEM.Page.setPage("basePosForm","pospageing",pageSize,pageNum,totalRecord,"getPosList");
        	 }else{
        		html+="<tr><td colspan='6' class='center'>没有相关数据</td></tr>";
        		$("#basePosTable tbody").append(html);
...
function delAccPos(id){
              JEEGEM.Model.confirm("确认移除人员职务吗？",function(){	
	              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/position/delAccPos',{accId:id},function(data){
		              JEEGEM.Model.info(data.resMsg,function(){getPosList(1);});
		});
	});	
}
