$(function () {
	...
	$('#addBtn').on('click', function(e) {
		...
		e.preventDefault();		
		cleanForm();
		GEM.Model.edit("auDiv","新增字典",function(){
			 if(GEM.Validate.form("auForm")){
				 ...
				 $.ajax({type:'POST',url:gempath+'/backstage/dataDict/add',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
			        	 if(data.res==1){
			        		 that.dialog("close");      
	...
	$('#itemformAdd').on('click', function(e){
		...
		e.preventDefault();
		cleanItemForm();
		GEM.Model.edit("itemDiv","新增字典字段",function(){
			 var html="";
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) GEM.Model.info("您没有选择任何内容!"); 
		GEM.Model.confirm("确认要删除选中的数据吗?",function(){	
			GEM.Ajax.doRequest(null,gempath +'/backstage/dataDict/delBatch',{chks:chks.toString()},function(data){
				GEM.Model.info(data.resMsg,function(){search();});
			});
		});		
	});
...
function delItem(id){
	GEM.Model.confirm("确认要删除字段吗?",function(){$("#temtr"+id).remove();});	
}
function editItem(id){
	...
	GEM.Model.edit("itemDiv","修改字典字段",function(){
		 var name=$("#itemDiv input[name$='name']").val();
		 var value=$("#itemDiv input[name$='value']").val();
		 var sort=$("#itemDiv input[name$='sort']").val();
		 if(GEM.Validate.form("itemDiv")){		
...
function getbaseList(init){
	...
	GEM.Model.loading();
	GEM.Ajax.doRequest("baseForm",gempath +'/backstage/dataDict/findByPage',null,function(data){
...
function del(id){
	GEM.Model.confirm("确认删除吗？",function(){	
		GEM.Ajax.doRequest(null,gempath +'/backstage/dataDict/del',{id:id},function(data){
		 	 GEM.Model.info(data.resMsg,function(){search();});
		});
	});
}
function edit(id){
	cleanForm();
	GEM.Ajax.doRequest(null,gempath +'/backstage/dataDict/find',{id:id},function(data){
		setForm(data);
		GEM.Model.edit("auDiv","修改字典",function(){
			if(GEM.Validate.form("auForm")){
				var that =$(this);
				...
				$.ajax({type:'POST',url:gempath+'/backstage/dataDict/update',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
					   if(data.res==1){
					      that.dialog("close");
					      GEM.Model.info(data.resMsg,function(){search();});
					   }else{
...
function check(id){
	...
	GEM.Ajax.doRequest(null,gempath +'/backstage/dataDict/find',{id:id},function(data){
		setForm(data);
		GEM.Model.check("auDiv","查看字典"); 	
	});
}
function setForm(data){
	var l=data.obj;
	$("#auForm input[name$='id']").val(l.id);
	$("#auForm input[name$='dataKey']").val(GEM.Object.notEmpty(l.dataKey));
