$(function () {
              JEEGEM.Dict.setSelect("selectlogType","logType",2,'全部');
		...
		if(chks.length==0) {
			 JEEGEM.Model.info("您没有选择任何内容!"); 
		              ...
		              JEEGEM.Model.confirm("确认要删除选中的数据吗?",function(){	
			              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/task/taskLog/delBatch',{chks:chks.toString()},function(data){
	...
	$('#exportXls').on('click', function(e) {
		//通知浏览器不要执行与事件关联的默认动作		
		e.preventDefault();
	              JEEGEM.Model.confirm("确认要导出数据吗?",function(){	
...
function getbaseList(init){
	if(init==1) $("#baseForm .pageNum").val(1);	
              ...
              JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/task/taskLog/findByPage',null,function(data){
