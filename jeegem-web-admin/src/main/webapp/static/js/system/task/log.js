$(function () {
	GEM.Dict.setSelect("selectlogType","logType",2,'全部');
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			GEM.Model.confirm("确认要删除选中的数据吗?",function(){	
				GEM.Ajax.doRequest(null,gempath +'/backstage/task/taskLog/delBatch',{chks:chks.toString()},function(data){
	...
	$('#exportXls').on('click', function(e) {
		//通知浏览器不要执行与事件关联的默认动作		
		e.preventDefault();
		GEM.Model.confirm("确认要导出数据吗?",function(){	
			window.location.href=gempath+"/backstage/task/taskLog/exportXls";
		});	
	});
});
function search(){
...
function getbaseList(init){
	...
	GEM.Model.loading();
	GEM.Ajax.doRequest("baseForm",gempath +'/backstage/task/taskLog/findByPage',null,function(data){
		 $("#baseTable tbody").empty();
    		 ...
    		 if(results!=null&&results.length>0){
        		 ...
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 html+="<td class='center hidden-480' >"+GEM.Object.notEmpty(l.className)+"</td>";
            		 ...
            		 html+="<td class='center '>"+GEM.Date.Default(l.createTime)+"</td>";
            		 html+="</tr>";		 
            	 } 
