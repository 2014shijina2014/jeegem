...
function loadPro(){
	var settings=$.cookie('GEM.settings');
	if(typeof(settings) != "undefined"){
		var sjson = JSON.parse(settings);	
		//切换窄屏 
		if(typeof(sjson.narWinMenu) != "undefined" &&sjson.narWinMenu=="1"){
			$("#main-container").addClass("container");
			$("#gem-settings-add-container").prop("checked",true);			
		}else{
			$("#main-container").removeClass("container");	
			$("#gem-settings-add-container").prop("checked",false);
		}
		//缩放菜单
		if(typeof(sjson.sfMenu) != "undefined" &&sjson.sfMenu=="1"){
			$("#sidebar").addClass("menu-min");
			$("#gem-settings-sfMenu").prop("checked",true);		
		}else{
			$("#sidebar").removeClass("menu-min");	
			$("#gem-settings-sfMenu").prop("checked",false);		
		}
		//切换右菜单
		if(typeof(sjson.posMenu) != "undefined" &&sjson.posMenu=="1"){
			$("#indexBody").addClass("rtl");
			$("#gem-settings-rtl").prop("checked",true);	
		}else{
			$("#indexBody").removeClass("rtl");	
			$("#gem-settings-rtl").prop("checked",false);	
		}
	}else{
		var sjson={};
		$.cookie('GEM.settings', JSON.stringify(sjson));
	}
}
function perSetting(){
	$("#perSetting").removeClass('hide').dialog({
		...
		buttons: [
		  {  
			...
			click: function() {	
				var that =$(this);
				 GEM.Ajax.doRequest("perSettingFrom",gempath +'/backstage/account/setSetting',null,function(data){
...
function savePro(){
	var checked=false;
	var sjson = JSON.parse($.cookie('GEM.settings'));
	//切换窄屏 
...
function perData(){
	//获取个人资料
	GEM.Ajax.doRequest(null,gempath +'/backstage/account/getPerData',null,function(data){
		if(data.res==1){
       		var o=data.obj;
       		//加载头像
       		$('#headpicRecommend li').removeClass("visiHot");
       		$("#headpicPreviewImg").attr('src',GEM.Object.notNull(o.picUrl)?(o.picUrl):(gempath+"/static/images/system/user/hpic0.jpg"));
       		//加载个人资料
       		$("#perDataloginName").empty().append(GEM.Object.notEmpty(o.loginName));
       		$("#perDataName").val(GEM.Object.notEmpty(o.name));
       		$("#perDataEmail").val(GEM.Object.notEmpty(o.email));   	
       		$("#perDataRole").empty().append(GEM.Object.notEmpty(o.roleName));   
       		$("#perDataloginIP").empty().append(GEM.Object.notEmpty(o.loginLog.loginIP));   
       		$("#perDataloginTime").empty().append(GEM.Date.Default(o.loginLog.loginTime));  
       		//打开窗体
       			  ...
       			  {  
       				...
       				click: function() {	
       					...
       					if($("#avatarSetting").hasClass("active")){
       						...
       						GEM.Ajax.doRequest(null,gempath +'/backstage/account/setHeadpic',{picUrl:picUrl},function(data){
	    				       GEM.Model.info(data.resMsg); 
	    				       $("#mainHeadpic").attr('src',picUrl);
	    				       that.dialog("close");
       						});
       					}else if($("#perfileSetting").hasClass("active")){
       						if(GEM.Validate.form("perfileSettingFrom")){	
	       						GEM.Ajax.doRequest("perfileSettingFrom",gempath +'/backstage/account/setPerData',null,function(data){
		       				        GEM.Model.info(data.resMsg); 
       						...
       						if(GEM.Validate.form("pwSettingFrom")){
       							...
       							if(opwd!=npwd){
       								if(npwd==qpwd){	
       									GEM.Ajax.doRequest(null,gempath +'/backstage/account/preResetPWD',{opwd:opwd,npwd:npwd,qpwd:qpwd},function(data){
			       				        	GEM.Model.info(data.resMsg); 
			       				        	that.dialog("close");	
       									});
           							}else{
...
function openMenu(type,id,parentId,menuName,resUrl){
		...
		else if('/'==resUrl.substring(0,1))TabControlAppend(id,menuName,gempath+resUrl+"?menu="+id);
		else TabControlAppend(id,menuName,resUrl);	
}
function getMenu(layer,ref){
	$("#menu_li_id").empty();
	GEM.Ajax.doRequest(null,gempath +'/backstage/menu/getMenu',{layer:layer,ref:ref},function(data){
    	$("#menu_li_id").html(data.obj); 
	});
}
function logout(){
	GEM.Model.confirm("确认要退出吗？",function(){window.location.href=gempath+"/system_logout";});	
}
