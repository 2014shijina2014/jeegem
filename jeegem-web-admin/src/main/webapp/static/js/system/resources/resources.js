$(function () {
	...
	$('#addBtn').on('click', function(e) {
		...
		var layer=$("#auForm input[name$='layer']").val();
	              JEEGEM.Ajax.doRequest("",jeegempath +'/backstage/resources/listResources',{layer:layer},function(data){
			   //获取数据
			   var zNodes=data.obj;
			   //设置
			   ...
			   $.fn.zTree.init($("#preResourcesTree"), setting, zNodes);
					 ...
					 if(JEEGEM.Validate.form("auForm")){
						 var that =$(this);
						 JEEGEM.Ajax.doRequest("auForm",jeegempath +'/backstage/resources/add',null,function(data){
				        	that.dialog("close");      
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			 JEEGEM.Model.info("您没有选择任何内容!"); 
				 ...
				 JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/resources/delBatch',{chks:chks.toString()},function(data){
				              JEEGEM.Model.info(data.resMsg,function(){search();});
				 });
			});	
		}	
...
function getbaseList(){
              ...
              JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/resources/listAllParentMenu',"",function(data){
...
function setNodes(id,parentid,nodes,html,level,permitBtn){
	if(nodes!=null&&nodes.length>0){
		...
		for(var i = 0;i<nodes.length;i++){
    		 ...
    		 if(i==nodes.length-1)
    			 html += "<img src='"+jeegempath+"/static/css/system/system/images/joinbottom.gif' class='joinMiddle'/>";
    		 else
    			 html += "<img src='"+jeegempath+"/static/css/system/system/images/join.gif' class='joinMiddle'/>";
...
function check(id){
	//清空表单
	cleanForm();
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/resources/find',{id:id},function(data){
    		setForm(data);  
	...
	var layer=$("#auForm input[name$='layer']").val();
		   ...
		   JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/resources/find',{id:id},function(data){
		    		setForm(data);  		
						 ...
						 if(JEEGEM.Validate.form("auForm")){
							 var that =$(this);
							 JEEGEM.Ajax.doRequest("auForm",jeegempath +'/backstage/resources/update',null,function(data){
					        		 that.dialog("close");
					        		 JEEGEM.Model.info(data.resMsg,function(){search();});		        		 	
							 });
						}
...
function setForm(data){
	...
	$("#preResources").val(JEEGEM.Object.notEmpty(l.parentName));
	var parentId=l.parentId;
	$("#auForm input[name$='parentId']").val(JEEGEM.Object.notEmpty(parentId));//上级资源
	...
	if(JEEGEM.Object.notNull(l.icon)){
		...
		$("#auForm input[name$='icon']").val(JEEGEM.Object.notEmpty(l.icon));//图标
	...
	$("#auForm select[name$='type']").val(JEEGEM.Object.notEmpty(l.type));//类型
	changeType(l.type);
	$("#auForm input[name$='resUrl']").val(JEEGEM.Object.notEmpty(l.resUrl));//路径
	$("#auForm input[name$='btnId']").val(JEEGEM.Object.notEmpty(l.btnId));//主键
	$("#auForm input[name$='btnFun']").val(JEEGEM.Object.notEmpty(l.btnFun));//方法
...
function del(id){
              JEEGEM.Model.confirm("确认删除吗？",function(){	
	              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/resources/del',{id:id},function(data){
		              JEEGEM.Model.info(data.resMsg,function(){search();});
		});
	});	
}
...
function selectIcon(){
	...
	if(JEEGEM.Object.notNull(icon)) $("#testIconId").prop("class",icon+" bigger-200");
