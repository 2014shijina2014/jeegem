...
function sendMail(){
	...
	if(GEM.Object.notNull(toList)){
		var tos=toList.split(";");
		for(var i=0;i<tos.length;i++){
			if(GEM.Object.notNull(tos[i].trim())&&!GEM.Validate.isEmail(tos[i])){
	...
	GEM.Model.loading();
	GEM.Ajax.doRequest(null,gempath +'/backstage/tool/email/sendMail',params,function(data){
...
function getbaseList(init){
	...
	GEM.Model.loading();
	GEM.Ajax.doRequest("baseForm",gempath +'/backstage/tool/email/findByPage',null,function(data){
		 $("#sentTable").empty();
    		 var obj=data.obj;
        	 var list=obj.list;
    		 ...
    		 if(results!=null&&results.length>0){
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 html+="<span title='"+GEM.Object.notEmpty(l.toList)+"' class='sender'>"+GEM.Object.notEmpty(l.toList)+"</span>";
            		 html+="<span class='time'>"+comtime(l.createTime)+"</span>";
            		 html+="<span class='summary'>";
            		 html+="<span class='text'>"+GEM.Object.notEmpty(l.subject)+"</span>";  
            		 html+="</div>";  
            	 } 
...
function getConfig(){
	...
	GEM.Model.loading();
	GEM.Ajax.doRequest(null,gempath +'/backstage/tool/email/getConfig',null,function(data){
			var mc=data.obj;
