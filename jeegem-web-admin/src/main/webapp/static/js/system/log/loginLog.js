	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			GEM.Model.confirm("确认要删除选中的数据吗?",function(){	
				GEM.Ajax.doRequest(null,gempath +'/backstage/loginLog/delBatch',{chks:chks.toString()},function(data){
...
function getbaseList(init){
	...
	GEM.Model.loading();
	GEM.Ajax.doRequest("baseForm",gempath +'/backstage/loginLog/findByPage',null,function(data){
		 $("#baseTable tbody").empty();
    		 ...
    		 if(results!=null&&results.length>0){
        		 ...
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 html+="<td class='center'>"+GEM.Object.notEmpty(l.aName)+"</td>";
            		 html+="<td class='center' >"+GEM.Object.notEmpty(l.loginIP)+"</td>";
            		 html+="<td class='center'>"+GEM.Date.Default(l.loginTime)+"</td>";
            		 html+="</tr>";		 
            	 } 
