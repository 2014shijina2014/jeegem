...
$(function () {
	...
	$('#itemformAdd').on('click', function(e){
		e.preventDefault();
		cleanItemForm();
		GEM.Model.edit("itemDiv","新增图文",function(){
			 var html="";
			 ...
			 if(GEM.Validate.form("itemDiv")){				 
				 html+="<tr id='temtr"+itemNum+"' >";
				 html+="<td>"+title+"</td>";
				 html+="<td>"+content+"</td>";
				 html+="<td>"+url+"</td>";
				 if(GEM.Object.notNull(picUrl)){
...
function delItem(id){
	GEM.Model.confirm("确认要删除该图文吗?",function(){$("#temtr"+id).remove();});	
}
function editItem(id){
	...
	GEM.Model.edit("itemDiv","修改图文",function(){
		 var title=$("#itemDiv input[name$='title']").val();
		 var content=$("#itemDiv input[name$='content']").val();
		 var sort=$("#itemDiv input[name$='sort']").val();
		 var url=$("#itemDiv input[name$='url']").val();
...
function refashMenu(){
	GEM.Model.loading();
	$("#menuList").empty();
	GEM.Ajax.doRequest(null,gempath +'/backstage/weixin/menu/menuTree',null,function(data){
...
function findMenu(id){
	cleanForm();
	GEM.Model.loading();
	GEM.Ajax.doRequest(null,gempath +'/backstage/weixin/menu/findMenu',{id:id},function(data){
			setForm(data);
		...
		if("text"==selectType){
			...
			if(items!=null && items.length>0){
				...
				for(var i=0;i<items.length;i++){
					var item=items[i];			 
					html+="<tr id='temtr"+itemNum+"' >";
					html+="<td>"+GEM.Object.notEmpty(item.title)+"</td>";
					html+="<td>"+GEM.Object.notEmpty(item.content)+"</td>";
					html+="<td>"+GEM.Object.notEmpty(item.url)+"</td>";
					if(GEM.Object.notNull(item.picUrl)){
					...
					html+="<li><a class='aBtnNoTD' onclick='delItem(&apos;"+itemNum+"&apos;)' title='删除' href='#'><i class='icon-remove-sign color-red bigger-120'></i></a></li>";	
					html+="</ul></div>";		
					html+="<input type='hidden' name='items'id='item"+itemNum+"' value='"+itemNum+"' itemTitle='"+GEM.Object.notEmpty(item.title)+"' itemUrl='"+GEM.Object.notEmpty(item.url)+"' itemSort='"+item.sort+"' itemPicUrl='"+GEM.Object.notEmpty(item.picUrl)+"' itemContent='"+GEM.Object.notEmpty(item.content)+"'  >";			
					html+="</td>";
					html+="</tr>";
					itemNum++;
				}
...
function saveMenu(){
	if(saveBtn=="add"){
		...
		GEM.Model.info("请选择菜单位置");
	}
}

function addMenu(){
	itemArry =new Array();
	if(GEM.Validate.form("menuForm")){
		 ...
		 $.ajax({type:'POST',url:gempath+'/backstage/weixin/menu/addMenu',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
        		GEM.Model.info(data.resMsg,function(){	
        			cleanForm();
        			refashMenu();	
        			saveBtn="no";
        		});	
...
function editMenu(){
	...
	if(GEM.Validate.form("menuForm")){
		 ...
		 $.ajax({type:'POST',url:gempath+'/backstage/weixin/menu/updateMenu',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
        		GEM.Model.info(data.resMsg,function(){	
        			cleanForm();
        			refashMenu();	
        			saveBtn="no";
...
function delMenu(){
	GEM.Model.confirm("确认要删除菜单吗?",function(){	
		var id=$("#menuForm input[name$='id']").val();
		GEM.Ajax.doRequest(null,gempath +'/backstage/weixin/menu/delMenu',{id:id},function(data){
				GEM.Model.info(data.resMsg,function(){
					cleanForm();
					refashMenu();
					saveBtn="no";
...
function syncMenu(){
	GEM.Model.confirm("确认要发布菜单吗?",function(){	
		GEM.Ajax.doRequest(null,gempath +'/backstage/weixin/menu/syncMenu',"",function(data){
			GEM.Model.info(data.resMsg);
		});
	});
}
function cleanItemForm(){
	GEM.Tags.cleanForm("itemDiv");
	$("#itemDiv input[name$='sort']").val('1');
}
