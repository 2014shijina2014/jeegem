...
$(function () {
	...
	$('#itemformAdd').on('click', function(e){
		e.preventDefault();
		cleanItemForm();
	              JEEGEM.Model.edit("itemDiv","新增图文",function(){
			 var html="";
			 ...
			 if(JEEGEM.Validate.form("itemDiv")){				 
				 html+="<tr id='temtr"+itemNum+"' >";
				 html+="<td>"+title+"</td>";
				 html+="<td>"+content+"</td>";
				 html+="<td>"+url+"</td>";
				 if(JEEGEM.Object.notNull(picUrl)){
...
function delItem(id){
              JEEGEM.Model.confirm("确认要删除该图文吗?",function(){$("#temtr"+id).remove();});	
}
function editItem(id){
	...
	$("#itemDiv input[name$='picUrl']").val(itemPicUrl);
              JEEGEM.Model.edit("itemDiv","修改图文",function(){
		 var title=$("#itemDiv input[name$='title']").val();
		 var content=$("#itemDiv input[name$='content']").val();
		 var sort=$("#itemDiv input[name$='sort']").val();
		 var url=$("#itemDiv input[name$='url']").val();
...
function refashMenu(){
	...
	$("#menuList").empty();
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/weixin/menu/menuTree',null,function(data){
...
function findMenu(id){
	cleanForm();
              JEEGEM.Model.loading();
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/weixin/menu/findMenu',{id:id},function(data){
			setForm(data);
...
function cleanForm(){
	...
	$("#delMenuBtn").addClass('hide');
              JEEGEM.Tags.cleanForm("menuForm");
...
function saveMenu(){
	if(saveBtn=="add"){
		...
		editMenu();
	              ...
	              JEEGEM.Model.info("请选择菜单位置");
	}
}

function addMenu(){
	itemArry =new Array();
	if(JEEGEM.Validate.form("menuForm")){
		 ...
		 $.ajax({type:'POST',url:gempath+'/backstage/weixin/menu/addMenu',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
        	              JEEGEM.Model.info(data.resMsg,function(){	
        			cleanForm();
        			refashMenu();	
        			saveBtn="no";
        		});	
...
function delMenu(){
              JEEGEM.Model.confirm("确认要删除菜单吗?",function(){	
		var id=$("#menuForm input[name$='id']").val();
	              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/weixin/menu/delMenu',{id:id},function(data){
			              JEEGEM.Model.info(data.resMsg,function(){
					cleanForm();
					refashMenu();
					saveBtn="no";
...
function syncMenu(){
              JEEGEM.Model.confirm("确认要发布菜单吗?",function(){	
	              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/weixin/menu/syncMenu',"",function(data){
		              JEEGEM.Model.info(data.resMsg);
		});
	});
}
function cleanItemForm(){
              JEEGEM.Tags.cleanForm("itemDiv");
	$("#itemDiv input[name$='sort']").val('1');
}
