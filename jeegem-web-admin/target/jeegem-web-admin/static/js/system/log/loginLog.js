	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JEEGEM.Model.confirm("确认要删除选中的数据吗?",function(){	
				JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/loginLog/delBatch',{chks:chks.toString()},function(data){
...
function getbaseList(init){
	...
	JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/loginLog/findByPage',null,function(data){
		 $("#baseTable tbody").empty();
    		 ...
    		 if(results!=null&&results.length>0){
        		 ...
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 html+="<td class='center'>"+JEEGEM.Object.notEmpty(l.aName)+"</td>";
            		 html+="<td class='center' >"+JEEGEM.Object.notEmpty(l.loginIP)+"</td>";
            		 html+="<td class='center'>"+JEEGEM.Date.Default(l.loginTime)+"</td>";
            		 html+="</tr>";		 
            	 } 
