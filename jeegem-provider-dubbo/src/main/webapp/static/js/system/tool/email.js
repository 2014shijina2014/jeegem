...
function sendMail(){
	...
	if(JEEGEM.Object.notNull(toList)){
		var tos=toList.split(";");
		for(var i=0;i<tos.length;i++){
			if(JEEGEM.Object.notNull(tos[i].trim())&&!GEM.Validate.isEmail(tos[i])){
	...
	var params={toList:toList,subject:subject,body:body};
              ...
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/tool/email/sendMail',params,function(data){
...
function getbaseList(init){
	if(init==1) $("#baseForm .pageNum").val(1);	
              JEEGEM.Model.loading();
              JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/tool/email/findByPage',null,function(data){
		 $("#sentTable").empty();
    		 var obj=data.obj;
        	 var list=obj.list;
...
function getConfig(){
              ...
              JEEGEM.Model.loading();
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/tool/email/getConfig',null,function(data){
			var mc=data.obj;
