//发送请求
function sendSever(){
	if(JEEGEM.Validate.form("interfaceForm")){
		...
		var startTime = new Date().getTime(); //请求开始时间  毫秒
		 JEEGEM.Ajax.doRequest("interfaceForm",jeegempath +'/backstage/tool/interfaceTest/severTest',null,function(data){
...
function reSetForm(){
	$("#interfaceForm input[name$='requestMethod'][value='POST']").prop("checked",true);
              JEEGEM.Tags.cleanForm("interfaceForm");
	$("#serverTime").val("-");
	$("#clientTime").val("-");
}