$(function () {
	...
	$("#authorityDiv button").bind('click', function(){		
		...
		if(auth=='org'){
			var orgId=$("#baseForm input[name$='orgId']").val();
		              JEEGEM.Ajax.doRequest("",jeegempath +'/backstage/org/role/orglistAuthorized',{id:orgId,layer:layer},function(data){
			...
			var roleId=$("#authorityDiv input[name$='roleId']").val();
		              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/listAuthorized',{id:roleId,layer:layer},function(data){
				$.fn.zTree.init($("#authorityTree"),{check: {enable: true,chkDisabledInherit: true,chkboxType:{"Y":"ps","N":"s"}},data:{simpleData:{enable: true}}},data.obj);	      
			});
		}else{
		              JEEGEM.Model.error("选择显示层级有误！");
		}
	});
	//授权机构组织
	$('#authOrgBtn').on('click', function(e) {
		...
		if(JEEGEM.Object.notNull(orgId)){
		              JEEGEM.Ajax.doRequest("",jeegempath +'/backstage/org/role/orglistAuthorized',{id:orgId,layer:layer},function(data){
			         $.fn.zTree.init($("#authorityTree"),{check: {enable: true,chkDisabledInherit: true,chkboxType:{"Y":"ps","N":"s"}},data:{simpleData:{enable: true}}},data.obj);	      
			         JEEGEM.Model.edit("authorityDiv","机构组织授权",function(){
			        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
		 					...
		 					layer=$("#authorityDiv input[name$='layer']").val();
		 				              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/saveOrgAuthorized',{id:orgId,aus:aus,layer:layer},function(data){
		 	 					 that.dialog("close");
	...
	$('#addOrgBtn').on('click', function(e) {
		...
		cleanOrgForm();
		loadPreOrgTree();
	              JEEGEM.Model.edit("auOrgDiv","新增机构组织",function(){
			 if(JEEGEM.Validate.form("auOrgForm")){
				 var that =$(this);
				 JEEGEM.Ajax.doRequest("auOrgForm",jeegempath +'/backstage/org/role/addOrg',null,function(data){
						 that.dialog("close"); 
		        		 JEEGEM.Model.info(data.resMsg,function(){refreshOrgTree();});
				 }); 
			 }	
		});
	});
	...
	$('#editOrgBtn').on('click', function(e) {
		...
		loadPreOrgTree();
		var orgId=$("#baseForm input[name$='orgId']").val();
	              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/role/findOrg',{id:orgId},function(data){
			if(data.res==1){
				setOrgForm(data);    
			              JEEGEM.Model.edit("auOrgDiv","修改机构组织",function(){		
					 if(JEEGEM.Validate.form("auOrgForm")){
						 var that =$(this);
						 JEEGEM.Ajax.doRequest("auOrgForm",jeegempath +'/backstage/org/role/updateOrg',null,function(data){
								 that.dialog("close"); 
				        		 JEEGEM.Model.info(data.resMsg,function(){refreshOrgTree();});     
						 }); 
					 }	
	...
	$('#delOrgBtn').on('click', function(e) {
		...
		var ptreeNode=treeObj.getNodeByParam("id",orgId,null);
	              JEEGEM.Model.confirm("确认要删除【"+ptreeNode.name+"】吗？",function(){		
		              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/role/delOrg',{id:orgId},function(data){
			              JEEGEM.Model.info(data.resMsg,function(){loadOrgTree();});
			});
		});	
	});
	...
	$('#addBtn').on('click', function(e) {
		...
		cleanForm();
		 ...
		 if(JEEGEM.Object.notNull(orgId)){
			/* var treeObj = $.fn.zTree.getZTreeObj("orgTree");
		     var ptreeNode=treeObj.getNodeByParam("id",orgId,null);*/
			              JEEGEM.Model.edit("auDiv","新增角色",function(){
				 if(JEEGEM.Validate.form("auForm")){
					 var that =$(this);			
					 JEEGEM.Ajax.doRequest("auForm",jeegempath +'/backstage/org/role/add',{orgId:orgId},function(data){
			        		 that.dialog("close");      
			        		 JEEGEM.Model.info(data.resMsg,function(){search();});
					 }); 
				 }	
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
		              ...
		              JEEGEM.Model.confirm("确认要删除选中的角色吗?",function(){
			              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/delBatch',{chks:chks.toString()},function(data){
				              JEEGEM.Model.info(data.resMsg,function(){search();});
				});
			});	
		}	
...
function loadOrgTree(){
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/getOrgTree',null,function(data){
		//设置
...
function loadPreOrgTree(){
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/getPreOrgTree',null,function(data){
		//设置
...
function clickOrg(event,treeId,treeNode) {
	...
	$("#orgName1").html("");
	$("#orgName2").html("");
	if(JEEGEM.Object.notNull(treeNode.pId)){
		var treeObj = $.fn.zTree.getZTreeObj("orgTree");
...
function getbaseList(init){
	if(init==1)$("#baseForm .pageNum").val(1);	
              JEEGEM.Model.loading();
              JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/org/role/findByPage',null,function(data){
...
function authorized(id){
	...
	var layer=$("#authorityDiv input[name$='layer']").val();
              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/listAuthorized',{id:id,layer:layer},function(data){
	         //获取数据
	    	 var zNodes=data.obj;
	         //设置
	         ...
	         JEEGEM.Model.edit("authorityDiv","角色授权",function(){
	        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
 					nodes = zTree.getCheckedNodes(),aus ="";
 					for (var i=0, l=nodes.length; i<l; i++) {
 						aus += nodes[i].id + ",";
 					...
 					var that =$(this);
 					layer=$("#authorityDiv input[name$='layer']").val();
 				              JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/org/role/saveAuthorized',{id:id,aus:aus,layer:layer},function(data){
 	 					  that.dialog("close");
 	 					  JEEGEM.Model.info(data.resMsg,function(){search();});			        		 	
 					});
	         });
...
function check(id){
	//清空表单
	cleanForm();
              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/role/find',{id:id},function(data){
		setForm(data);    
...
function edit(id){
	...
	cleanForm();	
              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/role/find',{id:id},function(data){
    		setForm(data);    
    	              JEEGEM.Model.edit("auDiv","修改角色",function(){
    			 if(JEEGEM.Validate.form("auForm")){
					 var that =$(this);
					 JEEGEM.Ajax.doRequest("auForm",jeegempath+'/backstage/org/role/update',null,function(data){
			        		 that.dialog("close");
			        		 JEEGEM.Model.info(data.resMsg,function(){search();});      				        		 	
					 });
				}
...
function del(id){
              JEEGEM.Model.confirm("确认删除角色吗？",function(){	
	              JEEGEM.Ajax.doRequest(null,jeegempath+'/backstage/org/role/del',{id:id},function(data){
			 JEEGEM.Model.info(data.resMsg,function(){search();});
		});
	});	
}
function cleanOrgForm(){
              JEEGEM.Tags.cleanForm("auOrgForm");
...
function setOrgForm(data){
	var l=data.obj;
	$("#auOrgForm input[name$='id']").val(JEEGEM.Object.notEmpty(l.id));
	$("#auOrgForm input[name$='name']").val(JEEGEM.Object.notEmpty(l.name));
	$("#auOrgForm textarea[name$='description']").val(JEEGEM.Object.notEmpty(l.description));//描述
	$("#preOrg").val(JEEGEM.Object.notEmpty(l.pName));
	$("#auForm input[name$='pId']").val(JEEGEM.Object.notEmpty(l.pId));//上级资源
};  
function setForm(data){
	var l=data.obj;
	$("#auForm input[name$='id']").val(JEEGEM.Object.notEmpty(l.id));
	...
	$("#auForm textarea[name$='description']").val(JEEGEM.Object.notEmpty(l.description));//描述
}
