$(function () {
	...
	$('#addBtn').on('click', function(e) {
		...
		e.preventDefault();		
		cleanForm();
		JEEGEM.Model.edit("auDiv","新增字典",function(){
			 if(JEEGEM.Validate.form("auForm")){
				 ...
				 $.ajax({type:'POST',url:jeegempath+'/backstage/dataDict/add',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
			        	 if(data.res==1){
			        		 that.dialog("close");      
	...
	$('#itemformAdd').on('click', function(e){
		...
		e.preventDefault();
		cleanItemForm();
		JEEGEM.Model.edit("itemDiv","新增字典字段",function(){
			 var html="";
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) JEEGEM.Model.info("您没有选择任何内容!"); 
		JEEGEM.Model.confirm("确认要删除选中的数据吗?",function(){	
			JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/dataDict/delBatch',{chks:chks.toString()},function(data){
				JEEGEM.Model.info(data.resMsg,function(){search();});
			});
		});		
	});
...
function delItem(id){
	JEEGEM.Model.confirm("确认要删除字段吗?",function(){$("#temtr"+id).remove();});	
}
function editItem(id){
	...
	JEEGEM.Model.edit("itemDiv","修改字典字段",function(){
		 var name=$("#itemDiv input[name$='name']").val();
		 var value=$("#itemDiv input[name$='value']").val();
		 var sort=$("#itemDiv input[name$='sort']").val();
		 if(JEEGEM.Validate.form("itemDiv")){		
...
function getbaseList(init){
	...
	JEEGEM.Ajax.doRequest("baseForm",jeegempath +'/backstage/dataDict/findByPage',null,function(data){
...
function del(id){
	JEEGEM.Model.confirm("确认删除吗？",function(){	
		JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/dataDict/del',{id:id},function(data){
		 	 JEEGEM.Model.info(data.resMsg,function(){search();});
		});
	});
}
function edit(id){
	cleanForm();
	JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/dataDict/find',{id:id},function(data){
		setForm(data);
		JEEGEM.Model.edit("auDiv","修改字典",function(){
			if(JEEGEM.Validate.form("auForm")){
				var that =$(this);
				...
				$.ajax({type:'POST',url:jeegempath+'/backstage/dataDict/update',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){  			        	 
					   if(data.res==1){
					      that.dialog("close");
					      JEEGEM.Model.info(data.resMsg,function(){search();});
					   }else{
...
function check(id){
	...
	JEEGEM.Ajax.doRequest(null,jeegempath +'/backstage/dataDict/find',{id:id},function(data){
		setForm(data);
		JEEGEM.Model.check("auDiv","查看字典"); 	
	});
}
function setForm(data){
	var l=data.obj;
	$("#auForm input[name$='id']").val(l.id);
	$("#auForm input[name$='dataKey']").val(JEEGEM.Object.notEmpty(l.dataKey));
